...
class RoundTest < Minitest::Test
  ...
  def test_round_has_turns
    ...
    new_turn = round2.take_turn("Juneau")
    assert_equal [], round1.turns
    assert_equal [new_turn], round2.turns
  end
  ...
  def test_round_current_card
    ...
    round = Round.new(deck)
    round2 = Round.new(deck)
    round2.take_turn("Juneau")
    assert_equal card_1, round.current_card
    assert_equal card_2, round2.current_card
  end

  def test_round_take_turn_exist
    card_1 = Card.new("What is the capital of Alaska?", "Juneau", :Geography)
    card_2 = Card.new("The Viking spacecraft sent back to Earth photographs and reports about the surface of which planet?", "Mars", :STEM)
    card_3 = Card.new("Describe in words the exact direction that is 697.5° clockwise from due north?", "North north west", :STEM)
    deck = Deck.new([card_1, card_2, card_3])
    ...
    assert_instance_of Turn, round2.take_turn("Venus")
  end
  ...
  def test_round_number_correct
    ...
    round2 = Round.new(deck)
    round2.take_turn("Juneau")
    round2.take_turn("Venus")
    assert_equal 1, round.number_correct
    assert_equal 1, round2.number_correct
  end
